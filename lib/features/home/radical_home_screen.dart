...
class _RadicalHomeScreenState extends State<RadicalHomeScreen> {
  ...
  Widget build(BuildContext context) {
    final pages = <Widget>[
      _HomeTab(onStart: _openLobby, avatar: _avatar, level: _level),
      _LobbyTab(onStart: _openLobby),
      const _StoreTab(),
      _ProfileTab(avatar: _avatar, level: _level, onAvatars: _openAvatarGallery),
    ];
  ...
  Widget build(BuildContext context) => ListView(
    ...
    children: [
      Row(children: [
        ...
        RealisticAvatar(role: _avatarRole(avatar), female: avatar.contains('زن'), size: 58),
      ]),
      ...
      Container(
        ...
        child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('ویژگی‌های رادیکال', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 17)),
          SizedBox(height: 10), Text('🎭 آواتارهای نقش‌محور', style: TextStyle(color: RadicalTheme.smoke)),
          SizedBox(height: 6), Text('🌙 فازهای شب و روز', style: TextStyle(color: RadicalTheme.smoke)),
          SizedBox(height: 6), Text('🏆 برد و پایان سینمایی', style: TextStyle(color: RadicalTheme.smoke)),
        ]),
      ),
    ],
  );
...
class _LobbyTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [
    const Icon(Icons.table_restaurant_rounded, size: 70, color: RadicalTheme.goldBright),
    const SizedBox(height: 14),
    const Text('لابی بازی', style: TextStyle(fontSize: 25, fontWeight: FontWeight.w900)),
    const SizedBox(height: 8),
    const Text('سناریو و تعداد بازیکن‌ها را انتخاب کن.', textAlign: TextAlign.center, style: TextStyle(color: RadicalTheme.smoke)),
    const SizedBox(height: 18),
    FilledButton.icon(onPressed: onStart, icon: const Icon(Icons.add_circle_outline), label: const Text('ساخت لابی')),
  ])));
}
...
class _StoreTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView(padding: const EdgeInsets.all(18), children: const [
    Text('فروشگاه رادیکال', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w900)),
    SizedBox(height: 8), Text('ظاهر شخصیتت را شخصی‌سازی کن.', style: TextStyle(color: RadicalTheme.smoke)),
    SizedBox(height: 20),
    _StoreInfo(title: 'فریم طلایی', icon: Icons.workspace_premium_rounded, text: 'فریم ویژه با درخشش طلایی برای پروفایل و میز بازی.'),
    _StoreInfo(title: 'شب خونین', icon: Icons.nights_stay_rounded, text: 'ظاهر تاریک و قرمز برای شخصیت‌های خاص.'),
    _StoreInfo(title: 'کلکسیون رادیکال', icon: Icons.auto_awesome_rounded, text: 'سیستم ظاهری در نسخه‌های بعدی گسترش پیدا می‌کند.'),
  ]);
}
...
class _AvatarSelectionScreenState extends State<AvatarSelectionScreen> {
  ...
  Widget build(BuildContext context) => Directionality(textDirection: TextDirection.rtl, child: Scaffold(
    backgroundColor: RadicalTheme.ink,
    appBar: AppBar(title: const Text('انتخاب آواتار', style: TextStyle(fontWeight: FontWeight.w900))),
    body: GridView.builder(
      padding: const EdgeInsets.all(18),
      gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 12, mainAxisSpacing: 12, childAspectRatio: .84),
      ...
      itemBuilder: (_, index) {
        ...
        return InkWell(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(color: active ? const Color(0xFF2A211A) : RadicalTheme.panel, borderRadius: BorderRadius.circular(24), border: Border.all(color: active ? RadicalTheme.goldBright : RadicalTheme.line, width: active ? 1.7 : 1)),
            child: Column(children: [
              Expanded(child: RealisticAvatar(role: _avatarRole(value), female: value.contains('زن'), size: 104)),
              const SizedBox(height: 8),
              Text(value, textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.w900, color: active ? RadicalTheme.goldBright : Colors.white)),
              const SizedBox(height: 5),
              Icon(active ? Icons.check_circle_rounded : Icons.radio_button_unchecked, color: active ? RadicalTheme.goldBright : RadicalTheme.smoke, size: 20),
            ]),
          ),
        );
      },
...
class _ProfileTab extends StatelessWidget {
  ...
  const _ProfileTab({required this.avatar, required this.level, required this.onAvatars});

  Future<Map<String, int>> _stats() async {
    final prefs = await SharedPreferences.getInstance();
    return {'xp': prefs.getInt('player_xp') ?? 0, 'played': prefs.getInt('games_played') ?? 0, 'won': prefs.getInt('games_won') ?? 0};
  }
  ...
  Widget build(BuildContext context) => FutureBuilder<Map<String, int>>(
    ...
    builder: (_, snapshot) {
      ...
      return ListView(padding: const EdgeInsets.fromLTRB(18, 22, 18, 30), children: [
        ...
        Container(padding: const EdgeInsets.all(20), decoration: RadicalTheme.glass(accent: true), child: Column(children: [
          RealisticAvatar(role: _avatarRole(avatar), female: avatar.contains('زن'), size: 112),
          const SizedBox(height: 12),
          const Text('بازیکن رادیکال', style: TextStyle(fontSize: 21, fontWeight: FontWeight.w900)),
          const SizedBox(height: 4),
          Text('سطح $level • XP ${stats['xp'] ?? 0}', style: const TextStyle(color: RadicalTheme.smoke)),
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: OutlinedButton.icon(onPressed: onAvatars, icon: const Icon(Icons.face_retouching_natural_rounded), label: const Text('تغییر آواتار'))),
        ])),
        const SizedBox(height: 14),
        Row(children: [_StatCard(title: 'بازی‌ها', value: '$played'), const SizedBox(width: 10), _StatCard(title: 'بردها', value: '$won'), const SizedBox(width: 10), _StatCard(title: 'نرخ برد', value: '$rate%')]),
        const SizedBox(height: 14),
        Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: RadicalTheme.panel, borderRadius: BorderRadius.circular(22), border: Border.all(color: RadicalTheme.line)), child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text('هویت بصری رادیکال', style: TextStyle(fontWeight: FontWeight.w900)),
          SizedBox(height: 7), Text('لوگو، طلایی/قرمز، کارت‌های شیشه‌ای و آواتارهای نقش‌محور.', style: TextStyle(color: RadicalTheme.smoke, height: 1.45)),
        ])),
      ]);
    },
  );
...
class _StoreInfo extends StatelessWidget {
  ...
  const _StoreInfo({required this.title, required this.icon, required this.text});
  @override
