                ...
                boxShadow: [
                  BoxShadow(
                    color: const Color(0xFFE3B873)
                        .withValues(alpha: .12 + glow * .18),
                    blurRadius: 22 + glow * 10,
                    spreadRadius: 1 + glow * 3,
                  ),
                  BoxShadow(
                    color: const Color(0xFF7C4DFF)
                        .withValues(alpha: .08 + glow * .14),
                    blurRadius: 28,
                  ),
                ],
              ),
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < 14; i++) {
      ...
      final particle = Paint()
        ..color = const Color(0xFFFFDFA0).withValues(alpha: alpha);
      canvas.drawCircle(
        point,
        i.isEven ? 1.25 + glowValue * .7 : .8 + glowValue * .35,
        particle,
      );
    }
    ...
    final drop = Paint()..color = const Color(0xFFB388FF);
    final dropPath = Path()
      ..moveTo(w * .50, h * .61)
    ...
    final shine = Paint()
      ..color = Colors.white.withValues(alpha: .07 + glowValue * .04);
    canvas.drawOval(
      Rect.fromLTWH(w * .24, h * .19, w * .20, h * .10),
      shine,
    );
  }
    ...
    switch (scenario) {
      case MafiaLogoScenario.darkCity:
        return const LinearGradient(
          ...
          colors: [Color(0xFF08090D), Color(0xFF26164A), Color(0xFF120B20)],
        );
