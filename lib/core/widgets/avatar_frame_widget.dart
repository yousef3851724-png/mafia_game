...
class AvatarFrameWidget extends StatelessWidget {
        ...
        children: [
          if (hasFrame && isAlive)
            Container(
              width: size + 10,
              height: size + 10,
              decoration: BoxDecoration(shape: BoxShape.circle, boxShadow: [BoxShadow(color: frameColor.withOpacity(.6), blurRadius: 12, spreadRadius: 2)]),
            ),
          if (hasFrame)
            Container(
              width: size + 8,
              height: size + 8,
              decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: isAlive ? frameColor : Colors.grey.shade700, width: 3.5)),
            ),
          Container(
            width: size,
            height: size,
            decoration: BoxDecoration(shape: BoxShape.circle, color: isAlive ? const Color(0xFF2A2A2A) : Colors.black87),
            child: ClipOval(
              child: imageUrl != null && imageUrl!.isNotEmpty
                  ? Image.network(imageUrl!, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _buildLocalFallback())
                  : _buildLocalFallback(),
            ),
          ),
          if (!isAlive)
  ...
  Widget _buildLocalFallback() {
    if (fallbackAsset != null && fallbackAsset!.isNotEmpty) {
      return SvgPicture.asset(fallbackAsset!, fit: BoxFit.cover);
    }
